...
function stockColumns(): Column<StockQuote>[] {
  return [
    ...
    { key: "price", header: "现价", align: "right", render: (s) => fmtPrice(s.price) },
    {
      key: "changePct",
      header: "涨跌",
      align: "right",
      render: (s) => <span className={cn("font-medium", changeColorClass(s.changePct))}>{fmtPct(s.changePct)}</span>,
    },
    { key: "pe", header: "PE", align: "right", render: (s) => (s.pe == null ? "—" : s.pe.toFixed(1)) },
    { key: "marketCap", header: "市值(亿)", align: "right", render: (s) => s.marketCap.toLocaleString("en-US") },
  ];
}
  ...
  return (
    <div className="mx-auto max-w-7xl p-6">
      ...
      <section className="mb-6">
        ...
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-7">
          {data.indices.map((idx) => (
            <StatCard key={idx.code} label={idx.name} value={fmtPrice(idx.price)} changePct={idx.changePct} />
          ))}
        </div>
      </section>
      ...
      <p className="mt-4 text-xs text-muted-foreground">
        ...
        {data.stale && <span className="ml-1 text-warning">（行情源暂不可用, 显示缓存示例数据）</span>}
      </p>
    </div>
  );
}
