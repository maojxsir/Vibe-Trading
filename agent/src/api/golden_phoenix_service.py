"""Golden Phoenix screener API service — read persisted results, status, and trigger scans."""
...
from src.screener.scan_golden_phoenix import (
    _config_params_snapshot,
    golden_phoenix_results_root,
    load_latest_result,
    load_result,
)
...
def golden_phoenix_state_root() -> Path:
    return Path.home() / ".vibe-trading" / "screener" / "golden_phoenix"


def status_path() -> Path:
    return golden_phoenix_state_root() / "status.json"


def lock_path() -> Path:
    return golden_phoenix_state_root() / "scan.lock"
...
def _scan_script_path() -> Path:
    return _agent_dir() / "scripts" / "run_golden_phoenix_scan.py"
...
def _read_status_file() -> dict[str, Any] | None:
    ...
    try:
        ...
        logger.warning("golden phoenix status read failed (%s): %s", path, exc)
        return None
...
def _result_policy_stale(raw: dict[str, Any]) -> bool:
    ...
    if not isinstance(params, dict):
        return True
    if params.get("policy_version") != GOLDEN_PHOENIX_POLICY_VERSION:
        return True
    expected = _config_params_snapshot(GoldenPhoenixConfig())
    for key in ("exclude_st", "min_callback_days", "max_callback_days"):
...
def get_golden_phoenix_payload(date: str = "") -> dict[str, Any]:
    query = (date or "").strip()
    if query:
...
def _release_lock() -> None:
    try:
        ...
        logger.warning("golden phoenix lock release failed: %s", exc)
...
def _read_lock_payload() -> dict[str, Any] | None:
    ...
    try:
        ...
        logger.warning("golden phoenix lock read failed (%s): %s", path, exc)
        return None
    return payload if isinstance(payload, dict) else None
...
def _clear_stale_lock(*, max_age_seconds: float = _STALE_LOCK_MAX_AGE_S) -> None:
    ...
    try:
        ...
        logger.warning("golden phoenix lock stat failed: %s", exc)
        return

    if age_seconds > max_age_seconds:
        logger.warning("golden phoenix stale lock removed (age %.0fs)", age_seconds)
        _release_lock()
        return
    ...
    if worker_pid and not _pid_is_alive(worker_pid):
        logger.warning("golden phoenix orphan lock removed (worker pid %s gone)", worker_pid)
        _release_lock()
        return

    if age_seconds > _ORPHAN_LOCK_GRACE_S and not worker_pid:
        logger.warning("golden phoenix legacy lock removed (age %.0fs)", age_seconds)
        _release_lock()
...
def _run_scan_worker() -> None:
    ...
    try:
        ...
        if proc.returncode != 0:
            ...
            if not detail:
                detail = f"golden phoenix exited with code {proc.returncode}"
            logger.error("golden phoenix subprocess failed: %s", detail[:1000])
        ...
        logger.error("golden phoenix subprocess timed out after %ss", _SCAN_TIMEOUT_S)
        _write_status(
            state="failed",
            progress=0,
            message=f"golden phoenix timed out after {_SCAN_TIMEOUT_S // 60} minutes",
        )
    except Exception as exc:  # noqa: BLE001
        logger.exception("golden phoenix refresh failed")
...
def trigger_refresh() -> tuple[int, dict[str, Any]]:
    with _STATE_LOCK:
        ...
        golden_phoenix_state_root().mkdir(parents=True, exist_ok=True)
        worker = threading.Thread(target=_run_scan_worker, name="golden-phoenix-scan", daemon=True)
...
def results_root() -> Path:
    return golden_phoenix_results_root()
